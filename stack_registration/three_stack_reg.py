...
def crop_initialize_reg(fixed_image, moving_image):
  ...
  moving_image_resampled = sitk.Resample(moving_image, 
                                fixed_image, 
                                initial_tfm, 
                                sitk.sitkBSpline, 
                                0.0, 
...
def register_stacks(fixed_image, moving_image, initial_tfm):
  ...
  # Optimizer settings
  reg.SetOptimizerAsGradientDescent(learningRate=5.0, 
...
def resample_full_extent(image, tmat):
  ...
  resampled_image = sitk.Resample(image, output_size, tmat, 
                                  sitk.sitkBSpline, output_origin, 
...
def three_stack_reg(top_stack, mid_stack, bottom_stack, output_dir):
  ...
  fixed_image1 = crop_image(mid_stack, 
                            [mid_stack.GetWidth(), mid_stack.GetHeight(), 42], 
                            [0, 0, mid_stack.GetDepth()-42])
  moving_image1 = crop_image(top_stack, 
                            [top_stack.GetWidth(), top_stack.GetHeight(), 42], 
                            [0, 0, 0])

  sitk.WriteImage(fixed_image1, midTop_overlap_path)
  sitk.WriteImage(moving_image1, top_overlap_path)
  
  top2Mid_initial_tfm, moving_image_resampled1 = crop_initialize_reg(fixed_image1, moving_image1)
  ...
  fixed_image2 = crop_image(mid_stack, 
                            [mid_stack.GetWidth(), mid_stack.GetHeight(), 42], 
                            [0, 0, 0])
  moving_image2 = crop_image(bottom_stack, 
  ...
  bottom2Mid_initial_tfm, moving_image_resampled2 = crop_initialize_reg(fixed_image2, moving_image2)
  ...
  # Registration #1: Top to Middle
  top2Mid_final_tfm = register_stacks(fixed_image1, moving_image1, top2Mid_initial_tfm, 'top')
  sitk.WriteTransform(top2Mid_final_tfm, top2Mid_final_tfm_path)

  # Registration #2: Bottom to Middle
  bottom2Mid_final_tfm = register_stacks(fixed_image2, moving_image2, bottom2Mid_initial_tfm, 'bottom')
  ...
  #-------------------------------------------------------------------#
  width = max( mid_stack.GetSize()[0], reg_top_image.GetSize()[0], reg_bottom_image.GetSize()[0] ) + 50
  height = max( mid_stack.GetSize()[1], reg_top_image.GetSize()[1], reg_bottom_image.GetSize()[1] ) + 50
  depth = ( mid_stack.GetSize()[2] + reg_top_image.GetSize()[2] + reg_bottom_image.GetSize()[2] - 42 - 42 )

  origin_x = min( reg_top_image.GetOrigin()[0], mid_stack.GetOrigin()[0], reg_bottom_image.GetOrigin()[0] )
  origin_y = min( reg_top_image.GetOrigin()[1], mid_stack.GetOrigin()[1], reg_bottom_image.GetOrigin()[1] )
  origin_z = min( reg_top_image.GetOrigin()[2], mid_stack.GetOrigin()[2], reg_bottom_image.GetOrigin()[2] )
  ...
  dest_index_x = abs(final_image.TransformPhysicalPointToIndex(final_image_origin)[0] - final_image.TransformPhysicalPointToIndex(mid_stack.GetOrigin())[0])
  dest_index_y = abs(final_image.TransformPhysicalPointToIndex(final_image_origin)[1] - final_image.TransformPhysicalPointToIndex(mid_stack.GetOrigin())[1])
  dest_index_z = abs(final_image.TransformPhysicalPointToIndex(final_image_origin)[2] - final_image.TransformPhysicalPointToIndex(mid_stack.GetOrigin())[2])
  mid_dest_index = [dest_index_x, dest_index_y, dest_index_z] 

  pasted_image = sitk.Paste(final_image, mid_stack, mid_stack.GetSize(), destinationIndex=mid_dest_index)
  ...
  top_mask = sitk.Resample(top_mask, pasted_image, interpolator=sitk.sitkNearestNeighbor)
  bottom_mask = sitk.Resample(bottom_mask, pasted_image, interpolator=sitk.sitkNearestNeighbor)
  ...
  reg_top_image = sitk.Resample(reg_top_image, pasted_image, interpolator=sitk.sitkBSpline)
  reg_bottom_image = sitk.Resample(reg_bottom_image, pasted_image, interpolator=sitk.sitkBSpline)
...
if __name__ == '__main__':
  ...
  parser.add_argument('mid_stack', type=str, help='Middle stack image (path + filename)')
  parser.add_argument('bottom_stack', type=str, help='Bottom stack image (path + filename)')
  ...
  if (not('.mha' in top_stack_basename or '.nii' in top_stack_basename)):
    sys.exit('Wrong file type for the top stack. Only MHA and NII images will be accepted.')
  elif (not('.mha' in bottom_stack_basename or '.nii' in bottom_stack_basename)):
    sys.exit('Wrong file type for the bottom stack. Only MHA and NII images will be accepted.')
  elif (not('.mha' in mid_stack_basename or '.nii' in mid_stack_basename)):
    sys.exit('Wrong file type for the middle stack. Only MHA and NII images will be accepted.')
