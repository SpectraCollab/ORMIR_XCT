...
def two_stack_reg(top_stack, bottom_stack, output_dir):
  ...
  fixed_image = sitk.RegionOfInterest( bottom_stack, 
                                      [bottom_stack.GetWidth(), 
                                      bottom_stack.GetHeight(), 42], 
                                      [0, 0, bottom_stack.GetDepth()-42] )
  sitk.WriteImage(fixed_image, bottom_overlap_path)

  moving_image = sitk.RegionOfInterest( top_stack, 
                                      [top_stack.GetWidth(), 
                                      top_stack.GetHeight(), 42], 
                                      [0, 0, 0] )
  sitk.WriteImage(moving_image, top_overlap_path)

  # Align images by centres
  initial_tfm = sitk.CenteredTransformInitializer(fixed_image, moving_image, 
  ...
  # Optimizer settings
  reg.SetOptimizerAsGradientDescent(learningRate=2.0, 
  ...
  extreme_points = [top_stack.TransformIndexToPhysicalPoint( (0, 0, 0) ), 
                    ...
                    top_stack.TransformIndexToPhysicalPoint( (top_stack.GetWidth(),top_stack.GetHeight(), 0) ),
                    top_stack.TransformIndexToPhysicalPoint( (top_stack.GetWidth(),0, top_stack.GetDepth()) ),
                    top_stack.TransformIndexToPhysicalPoint( (top_stack.GetWidth(),top_stack.GetHeight(), top_stack.GetDepth()) ),
                    top_stack.TransformIndexToPhysicalPoint( (0,top_stack.GetHeight(), top_stack.GetDepth()) ) ]

  # Use the inverse transform to get the bounds of the resampling grid
  final_tfm_inv = final_tfm.GetInverse()
  extreme_points_transf = [final_tfm_inv.TransformPoint(pnt) for pnt in extreme_points]
  ...
  reg_top_image = sitk.Resample(top_stack, output_size, final_tfm, 
  ...
  bottom_dest_index=[ abs(final_image.TransformPhysicalPointToIndex(final_image_origin)[0] - final_image.TransformPhysicalPointToIndex(bottom_stack.GetOrigin())[0]), 
                    abs(final_image.TransformPhysicalPointToIndex(final_image_origin)[1] - final_image.TransformPhysicalPointToIndex(bottom_stack.GetOrigin())[1]), 
                    abs(final_image.TransformPhysicalPointToIndex(final_image_origin)[2] - final_image.TransformPhysicalPointToIndex(bottom_stack.GetOrigin())[2]) ]

  pasted_image = sitk.Paste(final_image, bottom_stack, bottom_stack.GetSize(), destinationIndex=bottom_dest_index)
