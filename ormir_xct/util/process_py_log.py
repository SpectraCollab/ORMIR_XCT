...
def process_py_log(log_dir, output_dir):
    ...
    # Oversampled shapes
    hollow_sphere_oversample_files = glob.glob(os.path.join(log_dir, '*H_SPH*_OVERSAMPLE.txt'))
    filled_sphere_oversample_files = glob.glob(os.path.join(log_dir, '*F_SPH*_OVERSAMPLE.txt'))
    hollow_cylinder_oversample_files = glob.glob(os.path.join(log_dir, '*H_CYL*_OVERSAMPLE.txt'))
    filled_cylinder_oversample_files = glob.glob(os.path.join(log_dir, '*F_CYL*_OVERSAMPLE.txt'))
    ...
    hollow_sphere_files = list(set(hollow_sphere_files) - set(hollow_sphere_oversample_files))
    filled_sphere_files = list(set(filled_sphere_files) - set(filled_sphere_oversample_files))
    hollow_cylinder_files = list(set(hollow_cylinder_files) - set(hollow_cylinder_oversample_files))
    filled_cylinder_files = list(set(filled_cylinder_files) - set(filled_cylinder_oversample_files))
    ...
    csv_array = np.array(["Filename", "main_th", "main_th_sd", "main_th_min", 
                          "main_th_max"
                          ])
    ...
    for log_file in filled_cylinder_files:
        ...
        output_array = np.array([filename, main_th, main_th_sd, main_th_min, 
                                 main_th_max])
