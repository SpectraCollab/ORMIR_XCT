...
def create_filled_spheres(img_dir, padding=10,
                         min=1, max=51, inc=1):
    """
    ...
    for i in range(min, max, inc):
        ...
        sphere  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, sphere_outer_radius, shape_type="sphere")
...
def create_hollow_spheres(img_dir, padding=10,
                          out_min=1, out_max=51, out_inc=1,
                          in_min=1, in_max=51, in_inc=1):
    """
    ...
    for i in range(out_min, out_max, out_inc):
        ...
        for j in range(in_min, in_max, in_inc):
            ...
            outer_sphere  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, sphere_outer_radius, shape_type="sphere")
            inner_sphere  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, sphere_inner_radius, shape_type="sphere")
            ...
            img_name = os.path.join(img_dir, 'H_SPH_OUTR_' + str(i) + '_INR_' + str(j) + '.nii')
            sitk.WriteImage(img, img_name)


def create_filled_cylinders(img_dir, padding=10,
                            rad_min=1, rad_max=51, rad_inc=1,
                            len_min=1, len_max=101, len_inc=5):
    """
    ...
    for i in range(rad_min, rad_max, rad_inc):
        ...
        for l in range(len_min, len_max, len_inc):
            sphere_outer_radius = i * VOXEL_SIZE
            IMAGE_SHAPE = tuple([sphere_outer_radius*2, sphere_outer_radius*2, l*2])
            
            outer_cylinder  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, sphere_outer_radius, shape_type="cylinder")
            ...
            img_name = os.path.join(img_dir, 'F_CYL_OUTR_' + str(i) + '_LEN_' + str(l) + '.nii')
            sitk.WriteImage(img, img_name)


def create_hollow_cylinders(img_dir, padding=10,
                            out_min=1, out_max=51, out_inc=5,
                            in_min=1, in_max=51, in_inc=5,
                            len_min=1, len_max=101, len_inc=5):
    """
    ...
    for i in range(out_min, out_max, out_inc):
        ...
        for j in range(in_min, in_max, in_inc):
            ...
            for l in range(len_min, len_max, len_inc):
                ...
                outer_cylinder  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, sphere_outer_radius, shape_type="cylinder")
                inner_cylinder  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, sphere_inner_radius, shape_type="cylinder")
                hollow_cylinder = outer_cylinder - inner_cylinder

                print("Writing hollow cylinder: " + str(i) + "_" + str(j) + "_" + str(l))
                ...
                img_name = os.path.join(img_dir, 'H_CYL_OUTR_' + str(i) + '_INR_' + str(j) + '_LEN_' + str(l) + '.nii')
                sitk.WriteImage(img, img_name)
 

def create_plates(img_dir, padding=10,
                  w_min=1, w_max=51, w_inc=1,
                  l_min=1, l_max=51, l_inc=2):
    """
    ...
    for i in range(w_min, w_max, w_inc):
        ...
        for l in range(l_min, l_max, l_inc):
            ...
            plate  = create_shape(IMAGE_SHAPE, VOXEL_WIDTH, plate_width, shape_type="plate")
            ...
            img_name = os.path.join(img_dir, 'PLATE_WIDTH_' + str(i) + '_LEN_' + str(l) + '.nii')
            sitk.WriteImage(img, img_name)
...
def compute_thickness(base_dir):
    ...
    for filename in os.listdir(img_dir):
        ...
        log_file_name_oversample = os.path.join(log_dir, file_basename + "_OVERSAMPLE.txt")
        ...
        thickness_stats_branch = calc_structure_thickness_statistics(shape, tuple([VOXEL_SIZE]*3), VOXEL_SIZE)

        filename = file_basename + "_OVERSAMPLE"
        header = np.array(["shape", 
                            "th_mean", "th_sd", "th_min", "th_max"])
        content = np.array([filename, 
                            thickness_stats_branch[0], thickness_stats_branch[1], thickness_stats_branch[2], thickness_stats_branch[3]])
        ...
        thickness_stats_branch = calc_structure_thickness_statistics(shape, tuple([VOXEL_SIZE]*3), VOXEL_SIZE, oversample=False)

        header = np.array(["shape", 
                            "th_mean", "th_sd", "th_min", "th_max"])
        content = np.array([file_basename, 
                            thickness_stats_branch[0], thickness_stats_branch[1], thickness_stats_branch[2], thickness_stats_branch[3]])
        log_array = np.vstack((header, content))
