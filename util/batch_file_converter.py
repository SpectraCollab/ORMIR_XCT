...
def extension_check(input_dir, input_ext, output_ext):
    ...
    if 'aim' in input_ext :
        if (len(glob.glob(os.path.join(input_dir, '*.' + input_ext))) == 0 and
            len(glob.glob(os.path.join(input_dir, '*.' + input_ext + ';*'))) == 0 and
            len(glob.glob(os.path.join(input_dir, '*.' + input_ext.upper()))) == 0 and
            len(glob.glob(os.path.join(input_dir, '*.' + input_ext.upper() + ';*'))) == 0 ):
...
parser.add_argument('input_ext', type=str, help=\
                    'The input image file type (e.g. AIM, NII, MHA, etc.)')
parser.add_argument('output_ext', type=str, default='', help=\
                    'The output image file type (e.g. AIM, NII, MHA, etc.)')
